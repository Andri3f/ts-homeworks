...
function generateArray(holidays: string[], daysOff: string[], workDays: number[]): (number | string)[] {
	let schedule: (number | string)[] = []
	for (let i = 0; i < 10; i++) {
...
function countDays(schedule: (number | string)[], holidays: string[], daysOff: string[], workDays: number[]) {
	let holidaysCount = 0
	let daysOffCount = 0
	let workDaysCount = 0
...
const result4 = document.getElementById("result") as HTMLElement
result4.innerHTML = `
<div>
