function isWorkDayFromInput(input: string): boolean | null {
...
if (!userDayInput) {
	...
	if (res === null) alert("Невірне значення")
	else document.write(`${userDayInput} — ${res ? "робочий день" : "вихідний день"}`)
}
